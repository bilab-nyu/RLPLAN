...
# from likefurniture import layoutAI
from likefurniture_door_later_v3_randombox import layoutAI
# from likefurniture_door_later_v3 import layoutAI
from datetime import datetime

import os
os.environ["KMP_DUPLICATE_LIB_OK"]="TRUE"

class LayoutAIEnvironment(gym.Env):
    def __init__(self):
        self.game = layoutAI()
...
try:
    os.mkdir(f'save_mode_{now}')
except:
    pass

# MODEL_PATH = f"save_mode_{now}/pong-cnn-updated-"  # Models path for saving or loading
# MODEL_PATH = "save_mode_04-16-2023/pong-cnn-updated-"
# MODEL_PATH = "randomaction/pong-cnn-updated-"
MODEL_PATH = "yoyo/cnn-updated-"
...
environment = LayoutAIEnvironment()
...
for episode in range(startEpisode, MAX_EPISODE):
    ...
    for step in range(MAX_STEP):
        ...
        # select random action for paper

        action = agent.act(state)  # Act
        # action = environment.action_space.sample()

        next_state, reward, done, info = environment.step(action)  # Observe
        # print('check reward while training: ', reward)
        ...
        if done:  # Episode completed
            ...
            if SAVE_MODELS:
                outputPath = "out-out" + '.txt'  # Save outStr to file
